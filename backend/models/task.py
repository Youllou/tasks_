...
from sqlalchemy import DateTime, ForeignKey, String, Table, Column as SAColumn
...
task_tags = Table(
    "task_tags",
    Base.metadata,
    SAColumn("task_id", String, ForeignKey("tasks.id", ondelete="CASCADE"), primary_key=True),
    SAColumn("tag_id", String, ForeignKey("tags.id", ondelete="CASCADE"), primary_key=True),
)
...
class Task(Base):
    ...
    column_id: Mapped[str] = mapped_column(String, ForeignKey("columns.id", ondelete="RESTRICT"), nullable=False, index=True)
